...
Phaser.Plugin.GamygdalaExpression = function (game, sprite, agent, showOnlyMaxIntensity) {
	...
	this.map=[];
	this.map['distress']=0; // purple
	this.map['fear']=1; // purple
	this.map['hope']=2; // yellow
	this.map['joy']=3; // yellow
	this.map['satisfaction']=4;// green
	this.map['fear-confirmed']=5; // purple
	this.map['disappointment']=6; // blue
	this.map['relief']=7; // yellow
	this.map['happy-for']=8; //
	this.map['resentment']=9; // red
	this.map['pity']=10; // green
	this.map['gloating']=11; // red
	this.map['gratitude']=12; // green
	this.map['anger']=13; // red
	this.map['gratification']=14; // green
	this.map['remorse']=15; // blue
